...
public class AppGameContainer extends GameContainer {
	...
	public void setIcons(String[] refs) throws SlickException {
		GLFWImage.Buffer iconBuff = GLFWImage.malloc(refs.length);
		for (int i=0;i<refs.length;i++) {
			...
			try {
				GLFWImage image = data.loadImageAsGLFWImage(ResourceLoader.getResourceAsStream(refs[i]), flip, false, null);
				iconBuff.put(i, image);
			} catch (Exception e) {
				Log.error(e);
		...
		glfwSetWindowIcon(windowId, iconBuff);
	}
